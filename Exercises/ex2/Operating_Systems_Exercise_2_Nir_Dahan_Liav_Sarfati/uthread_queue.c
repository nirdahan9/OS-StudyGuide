...
{
    next_in_queue[tid] = -1;  // New node is always at the tail
    
    if (ready_queue_head == -1) { // Queue is empty
        ready_queue_head = tid;
        ready_queue_tail = tid;
    } else {
        // Queue is non-empty: link from current tail
...
void remove_from_ready(int tid)
{
    ...
    while (current != -1) {
        ...
        // If next is the thread to remove then unlink it
        if (next == tid) {
...
extern int current_tid;  // Import from uthreads.c to track which thread is running

static sigset_t set;
static sigset_t oldset_per_thread[MAX_THREAD_NUM];  // One saved mask per thread
...
{
    // Restore the old signal mask for the current thread
